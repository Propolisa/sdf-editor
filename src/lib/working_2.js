...
export function compileScene(
 ...
 if (typeName === 'SdResult') {
   // pass thenExpr first so selectSdResult(then, else, cond) ==> cond ? then : else
   return `selectSdResult(${thenExpr}, ${elseExpr}, ${cond})`;
 } else {
   // WGSL's built-in select(a, b, cond) is cond ? b : a, so this stays the same
...
`;
  ...
  function walk(node, scale, inVar) {
    ...
    for (let mod of node.modifiers || []) {
      ...
      lines.push(P.assign(pVar, P.call(mod.op, args)));
         if (mod.op === 'opScale') {
      // emit code to scale the distance in WGSL/glsl
      lines.push(P.assign('scale', `scale * ${P.fmtVal(mod.args.s)}`));
      // ***and*** update our JS-level scale for the recursion***
      scale *= mod.args.s;
    } else if (mod.op === 'opTransform') {
      const inv = mod.args.transform[0];
      const uniformScale = 1 / inv;
      lines.push(P.assign('scale', `scale * ${uniformScale.toFixed(6)}`));
      // apply the same to our JS variable
      scale *= uniformScale;
    }
    }
    ...
    if (isBool) {
      // recurse children
      const childVars = (node.children || []).map(ch => walk(ch, scale, pVar));
      ...
      if (!with_materials) {
        ...
        const def = SD_LIB[node.op];
        // take the raw k (or s) and then *scale* it into world‐space
        const kBase = node.args.k != null
        ? node.args.k
        : node.args.s || 0.0;
      // now *generate* code that multiplies runtime `scale` by this kBase:
      const kLit = `scale * ${P.fmtVal(kBase)}`;
        ...
        rest.forEach(childVar => {
          ...
          lines.push(
            P.assign(
              accR, 
              `SdResult(${tmpR}.dist, ${tmpR}.blend, ${P.fmtVal([0,0,0])}, 1.0, ${tmpC}, ${tmpID})`
            )
          );
  ...
  lines.push(`  ${rootVar}.dist = ${rootVar}.dist * scale;`);
